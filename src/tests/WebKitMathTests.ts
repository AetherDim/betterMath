QUnit.module("WebKit 15.8");

function shouldBe(a: string, b: string) {
	QUnit.test(a + " = " + b, assert => {
		assert.equal(eval(a), eval(b));
	});
}

function shouldBeTrue(a: string) {
	QUnit.test(a, assert => {
		assert.true(eval(a))
	});
}

function shouldBeFalse(a: string) {
	QUnit.test(a, assert => {
		assert.false(eval(a))
	});
}

function shouldThrow(a: string, error: string) {
	QUnit.test(a, assert => {
		try {
			eval(a)
			assert.true(false)
		} catch (err) {
			assert.strictEqual(err, eval(error))
		}
	});
} 
...
export {}
declare global {
    export var sideEffect: number
	export var testStr: string
}
globalThis.sideEffect = 0
globalThis.testStr = ''
...
function global() { return globalThis; }
